{{--
    View override, not a vendor patch — see `resources/views/vendor/filament/assets.blade.php`
    for the full rationale (SEC-08/CSP follow-up, 26 Aug 2026) and mechanism.

    Vendored from filament/filament v5.7.3's `vendor/filament/filament/resources/views/
    livewire/sidebar.blade.php`, content otherwise byte-for-byte identical — only the one
    `nonce="..."` attribute below was added, to the inline `<script>` that pre-collapses
    previously-collapsed navigation groups before Alpine boots. Only rendered on authenticated
    panel pages (the sidebar isn't part of the login page), which is why the earlier live audit
    caught it separately from the two `base.blade.php`/`assets.blade.php` tags present everywhere.
    MUST be re-diffed against the installed version's copy of this file on every
    `filament/filament` upgrade (`composer.lock` "filament/filament" version).
--}}
<div>
    @php
