{{--
    resources/views/components/icon/slash.blade.php

    <x-dynamic-component :component="'icon.' . $icon" ... /> — the glyph
    design-system.md §3.7 assigns to `DIBATALKAN` in BOTH families
    (intent `neutral`, "Terminal, not an error"). Reached through
    App\Support\Design\StatusIntent::icon(), never by a component
    switching on the enum itself (§3.7, §9.2 MUST #5).

    Provenance: the real, unmodified Heroicons v2 outline "SlashIcon"
    (heroicons 2.2.0, `24/outline/slash.svg` — MIT-licensed, 24x24
    ...
    Worth stating because it is easy to "fix" wrongly: `slash` is a REAL
    Heroicons v2 icon (a bare diagonal stroke, added in the 2.x line and
    present in 2.2.0), NOT shorthand for `no-symbol` (the circle-with-bar
    "prohibited" glyph, which also exists separately). The bare stroke is
    the correct reading here — §3.7 explicitly frames `DIBATALKAN` as
    "not an error", and `no-symbol` reads as prohibition. Verified
    against heroicons 2.2.0; do not substitute.
    ...
    locate a class or view for component [icon.slash]`), NOT as a
