{{--
    ...
    design-system.md §3.7 assigns to every `pending` status in both
    families: `MENUNGGU_KETERSEDIAAN`, `MENUNGGU_PEMBAYARAN` and
    `MENUNGGU_VERIFIKASI_PEMBAYARAN` (order lifecycle) plus
    `MENUNGGU_VENDOR` (vendor processing). Reached through
    App\Support\Design\StatusIntent::icon(), never by a component
    switching on the enum itself (§3.7, §9.2 MUST #5).

    Note for reviewers: `MENUNGGU_VERIFIKASI_PEMBAYARAN` deliberately
    shares this pending clock rather than a success glyph — §3.7 is
    normative and explicit there ("Never `success`"). That is a mapping
    decision owned by StatusIntent/§3.7, not by this file.
    ...
    locate a class or view for component [icon.clock]`), NOT as a
