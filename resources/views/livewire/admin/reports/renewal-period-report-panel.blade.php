{{--
    resources/views/livewire/admin/reports/renewal-period-report-panel.blade.php

    View for App\Livewire\Admin\Reports\RenewalPeriodReportPanel — the
    "Laporan Perpanjangan" tab inside App\Filament\Admin\Pages\Reports.
    Content moved verbatim from the former renewal-period-report.blade.php
    (the standalone RenewalPeriodReport Filament page); only the outer
    <x-filament-panels::page> wrapper was dropped — see
    finance-report-panel.blade.php's doc block for why.

    `RenewalStatus`'s own doc block: renewal status badges render through
    `StatusIntent`'s `FAMILY_ORDER_LIFECYCLE` map, the same family orders use
    — a renewal has no separate family of its own.
--}}
<div class="grid gap-y-6">
    <div class="grid gap-y-3">
        <div class="grid gap-y-1.5">
            <label for="renewal-period-report-period" class="text-sm font-medium text-neutral-800">
                Periode
            </label>
            <p class="text-sm text-neutral-600">
                Jumlah perpanjangan per status yang jatuh tempo pada periode tersebut (format YYYY-MM).
            </p>
            <div class="flex flex-wrap items-center gap-2">
                <x-filament::input.wrapper :valid="! $errors->has('period')">
                    <x-filament::input
                        id="renewal-period-report-period"
                        type="text"
                        inputmode="numeric"
                        wire:model="period"
                        maxlength="7"
    ...
    @elseif ($error === '')
        <div class="rounded-lg border border-neutral-200 bg-neutral-0 p-4">
            <p class="text-sm text-neutral-600">
                Belum ada perpanjangan jatuh tempo pada periode ini
            </p>
        </div>
    @endif
