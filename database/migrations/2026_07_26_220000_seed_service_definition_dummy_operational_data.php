 ...
 * values, not real ones. The values themselves are defined and generated by
 * `App\Support\ExampleData\ServiceOperationalExampleData`
 * (`operationalDefaults()` for the per-code rules, `dummyPrices()` for the
 * flat placeholder amounts); this migration is a thin shim that materializes
 * them.
 *
 * ---------------------------------------------------------------------------
 ...
 * service card. Every value in `ServiceOperationalExampleData` is a
 * reasoned, documented guess about
 ...
 * enters real owners/flags/prices, at which point the seeded values are
 * simply overwritten the normal way (owner/flags: a plain column update;
 * price: a new `RecordServiceDefinitionPriceVersion` call superseding the
 * seed row — the mechanism a real admin action uses).
 *
 * ---------------------------------------------------------------------------
 ...
 *
 * The fix is a direct `DB::table('price_versions')->insert()` in
 * `ServiceOperationalExampleData::seed()`: same
 ...
 *
 * Amounts are plausible Rupiah placeholders derived deterministically in
 * `ServiceOperationalExampleData::dummyPrices()` (a flat base plus a
 * per-catalogue-position step), one flat amount per service (matching every
 * other seeded
 * price's shape — a single amount snapshotted per booked line item).
 * Every service — CATERING included — is deliberately recorded as a flat
 * package price rather than a per-porsi/unit
 * price: `price_versions.amount` has no unit/quantity
