...
final class BookingWizard extends Component
{
    ...
    public function openOnlinePayment(): void
    {
        ...
        try {
            ...
            // submitted inside `OpenBookingOnlinePayment` a moment ago, and
            // it is simply waiting for the operator to confirm availability
            // and issue a final quote. Reporting that as a failure told
            // customers their booking had not gone through when it had —
            // the real defect behind the 8 Sep 2026 report.
            //
            ...
            //
            // ARCH-01 note: the order reference in the pending copy comes
            // from the denial itself (`OpenBookingOnlinePayment` attaches it
            // on the way out), not from this component re-querying for the
            // order it no longer submits. If it is somehow absent, the copy
            // degrades to the reference-free sentence rather than printing
            // an empty slot.
            if ($this->deniedOnlyByOperatorOwnedConditions($denial)) {
                $reference = $denial->orderReference();

                $this->onlinePaymentPendingNotice = $reference !== null
                    ? 'Pesanan Anda sudah kami terima dengan nomor '.$reference
                    ...
                    : 'Pesanan Anda sudah kami terima. Pembayaran dibuka setelah tim kami mengonfirmasi ketersediaan makam dan menerbitkan penawaran final — kami akan menghubungi Anda.';
            } else {
