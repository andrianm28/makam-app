...
/**
 * E2E-BOOK — the 4-screen public booking wizard (`/pemesanan-makam`).
 * `docs/testing/test-strategy.md` §2 `E2E-BOOK` acceptance criteria.
 *
 * Renumbered from nine saved steps to four
 * (`App\Domain\Booking\BookingWizardStep`: DISCOVERY=1,
 * CUSTOMER_AND_DECEASED_DATA=2, PAYMENT=3, CONFIRMATION=4;
 * `docs/superpowers/specs/2026-09-02-wizard-step-reduction-design.md`).
 * DISCOVERY merges the old city/TPU-TPS/service-type/services steps into ONE
 * screen with progressive-reveal local state and a single save
 * (`BookingWizard::continueFromDiscovery()`); the old Ringkasan (Summary)
 * step is now an always-visible panel on the same screen as the merged
 * customer+deceased form, not a numbered step of its own.
 *
 ...
 *   - Service types:     App\Domain\Booking\BookingServiceType
 ...
 *
 * Screen headings carry a stable `id="booking-step-N-heading"` anchor
 * (wizard.blade.php); every screen transition is asserted against that id
 * rather than translated copy, the same discipline `smoke.spec.ts` uses for
 * the homepage heading. Sub-sections WITHIN DISCOVERY (not their own
 * screens) carry their own `id="discovery-*-heading"` anchors instead.
 *
 * The step-completion helpers and fixture consts (`CUSTOMER`, `DECEASED`,
 * `startAtStep1`, `selectCity`, ...) live in `./e2e-booking-helpers.ts`
...
test.describe('E2E-BOOK — full journey (Makam Baru, standard branch)', () => {
    test('a visitor completes all 4 screens end to end with real fixture data', async ({ page }) => {
        await test.step('Screen 1 — Cari & Pilih: location', async () => {
            await startAtStep1(page);

            // Progress indicator: current step is 1 of 4, announced via the
        ...
        await test.step('Screen 1 — Cari & Pilih: TPU/TPS (no package cemetery)', async () => {
            ...
            // whole-card button. Still DISCOVERY — nothing is saved yet.
        ...
        await test.step('Screen 1 — Cari & Pilih: service type (Makam Baru / NEW_GRAVE)', async () => {
            ...
            // Scoped to the "Pilih Jenis Layanan" section (same real
            // collision `selectServiceType()` in e2e-booking-helpers.ts
            // documents: the cemetery listing above can render its own
            // "Makam Tumpang" package button, which stays visible alongside
            // this section under progressive reveal).
            const serviceTypeSection = page.getByLabel('Pilih Jenis Layanan');
            for (const label of ['Makam Baru', 'Makam Tumpang', 'Urgent', 'Pre-Need']) {
                await expect(serviceTypeSection.getByRole('button', { name: label, exact: true })).toBeVisible();
            }
        ...
        await test.step('Screen 1 — Cari & Pilih: real service catalog, mandatory + one additional, then save', async () => {
            ...
            // "Lanjutkan" — the ONE save for the entire DISCOVERY screen
            // (BookingWizard::continueFromDiscovery() -> saveStep1()).
        ...
        await test.step('Screen 2 — Detail Pemesanan: quote summary + customer & deceased form', async () => {
            ...
            // Ringkasan is no longer a numbered step — it is an always-visible
            // panel on this same screen, above the merged customer+deceased
            // form. Real seeded prices
            // (App\Support\ExampleData\ServiceOperationalExampleData
            // ::dummyPrices(): 350_000 + index*200_000 over
            // ServiceCode::KNOWN_CODES). DOCUMENT_PROCESSING (i=0) =
            // Rp 350.000, GRAVE_DIGGING (i=1) = Rp 550.000, AMBULANCE (i=2)
            // = Rp 750.000 — verified directly against the running database
            // before writing this assertion. <x-mk.table> renders BOTH a
            // real <table> (md+) and a <dl> mobile-card fallback in the DOM
            // simultaneously (one hidden via responsive classes at this
            // viewport), so text assertions are scoped to the real table to
        ...
        await test.step('Screen 3 — Pembayaran, manual fallback', async () => {
            ...
            await completeStep3Manual(page, 'BCA-TRF-000123-CONTOH');
        });
...
test.describe('E2E-BOOK — progress indicator and back/forward navigation', () => {
    test('the stepper reflects the current screen and back navigation preserves data', async ({ page }) => {
        await startAtStep1(page);
        await selectCity(page, 'Bogor');
        ...
        await selectCemeteryNoPackage(page, 'TPU Bogor 3');
        await selectServiceType(page, 'Makam Baru');

        const progress = page.getByRole('progressbar').first();
        // Still on DISCOVERY (screen 1): city/cemetery/service-type are all
        // local, non-persisting selections that share ONE save with the
        // services sub-choice, which has not run yet.
        await expect(progress).toHaveAttribute('aria-valuenow', '1');

        await continueFromDiscovery(page);
        await expect(progress).toHaveAttribute('aria-valuenow', '2');
        await expect(progress).toHaveAttribute('aria-valuetext', /Langkah 2 dari 4/);

        // Back navigation via Screen 2's own "Kembali" control, back to
        // DISCOVERY. There is only one "Kembali" control on Screen 2 (unlike
        // the old nine-step model, DISCOVERY's own sub-sections have none —
        // wizard.blade.php's own comment: "going back is simply clicking a
        // different city/TPU-TPS/jenis layanan").
        await page.getByRole('button', { name: 'Kembali', exact: true }).click();
        await expect(page.locator('#booking-step-1-heading')).toBeVisible();
        await expect(progress).toHaveAttribute('aria-valuenow', '1');

        // DISCOVERY's earlier selections are restored from the saved draft
        // (BookingWizard::hydrateFrom()), not lost by the round trip.
        await expect(page.getByRole('button', { name: 'Bogor', exact: true })).toHaveAttribute('aria-current', 'step');
        await expect(page.getByRole('button', { name: 'Terpilih TPU Bogor 3' })).toBeVisible();

        // Forward again — the draft is not corrupted by the round trip.
        await continueFromDiscovery(page);
        await expect(progress).toHaveAttribute('aria-valuenow', '2');
        ...
        // Screen 3's own "Kembali" returns to Screen 2 with the just-entered
        // customer data still populated — proving the autosaved draft, not
        // client-only form state, is what Screen 2 renders on return.
        await page.getByRole('button', { name: 'Kembali', exact: true }).click();
        ...
        // control (design-system.md §3.9: "complete -> clickable"). DISCOVERY
        // (step 1) is complete at this point in the journey.
        const step1Dot = page.getByRole('button', { name: /Langkah 1:.*selesai/ });
...
test.describe('E2E-BOOK — autosave and resume', () => {
    test('navigating away and back to the draft URL resumes at the saved step', async ({ page }) => {
        await startAtStep1(page);
        await selectCity(page, 'Tangerang');
        await selectCemeteryNoPackage(page, 'TPU Tangerang 7');
        await selectServiceType(page, 'Makam Baru');

        // DISCOVERY's four sub-choices share ONE save — no draft (and no
        // resumable URL) exists until `continueFromDiscovery()` runs, unlike
        // the old nine-step model where the city step alone created one.
        await continueFromDiscovery(page);
...
test.describe('E2E-BOOK — TPU/TPS package selection', () => {
    test('a cemetery with package rows requires choosing a package, not the whole card', async ({ page }) => {
        ...
        // Still DISCOVERY — a package pick reveals the next sub-section on
        // the SAME screen, it does not advance the stepper.
...
test.describe('E2E-BOOK — service type conditional branches', () => {
     ...
     * precondition on either value," and the services sub-section reads only
     ...
     * choice itself and the services sub-section rendering identically
     * afterward. One full journey already covers the NEW_GRAVE (Makam Baru)
     * branch above; these three spot-check that the other three choices are
     * each selectable, are correctly labelled, and reveal the same mandatory
     * services — not four redundant full end-to-end runs of behaviour that
     * does not yet diverge.
     */
    for (const [serviceTypeLabel, city, cemeteryName] of [
        ...
        test(`"${serviceTypeLabel}" is selectable and the mandatory services remain selected`, async ({ page }) => {
            await startAtStep1(page);
            await selectCity(page, city);
            await selectCemeteryNoPackage(page, cemeteryName);
            await selectServiceType(page, serviceTypeLabel);
