...
The recent-auth window is configuration-backed and security-approved (`config('reauthentication.freshness_seconds')`, 900 seconds by default). Attempts against the password challenge are rate-limited by `App\Platform\IdentityAccess\Reauthentication\ReauthenticationRateLimiter`.
