...
**(Rev.)** Daftar Lokasi Makam memakai direktori yang sudah ada di `/pemakaman`, Hubungi Kami memakai pusat bantuan di `/bantuan`, FAQ di `/faq`, Masuk di `/masuk`, Akun Saya di `/akun`. Hanya Tentang Kami yang halaman baru, berisi identitas YIEM sebagai penyelenggara dan legalitasnya.
...
**(Rev.)** Rilis 1 hanya mencatat data mentah untuk dua metrik yang sudah bisa dihitung dari order dan ledger: waktu dari draft pertama sampai konfirmasi, dan rasio invoice terbayar. Tidak ada dashboard metrik sampai target dikunci YIEM. Conversion dan attach rate menyusul setelah ada analitik.
...
Tiga puluh pertanyaan diajukan dalam empat ronde dan dijawab pemilik proyek. Semua mengikuti rekomendasi. Dicatat di sini agar revisi di atas punya alasan yang bisa ditelusuri.
...
Perubahan berikut diperlukan agar kode mengikuti PRD yang direkonsiliasi. Masing-masing lewat amandemen spec pemiliknya, lalu rencana Superpowers sendiri.

| Pekerjaan | Spec pemilik | Keputusan |
| --- | --- | --- |
| Kolom fasilitas tertutup dan filter fasilitas di direktori | `cemetery-directory-and-availability` | Q14, Q24, Q30 |
| Kontak pengelola dan daftar dokumen sebagai atribut Cemetery, field admin, proyeksi ke konfirmasi dan email | `cemetery-directory-and-availability`, `booking-and-order-orchestration`, `platform-notifications` | Q15, Q22, Q23 |
| Halaman informasi Wakaf Tanah dan halaman Tentang Kami; CTA sekunder Wakaf Tanah di hero | `public-home-and-navigation` | Q2, Q12, Q19, Q25, Q26 |
| Scheduler pengingat H-90, H-30, H-7 dengan idempotency per makam per jendela | `renewal-and-grave-registry` | Q7, Q17, Q18 |
| Badge terverifikasi dari capability profile aktif | `cemetery-directory-and-availability` | Q10 |
| Pencatatan data mentah waktu penyelesaian booking dan rasio invoice terbayar | `admin-operations` | Q28 |
