...
`/wakaf-tanah` (ditambahkan 24 Sep 2026, `.kiro/specs/public-home-and-navigation` AC12; `App\Livewire\Public\Legal\WakafTanah`, route name `legal.wakaf-tanah`) adalah halaman informasi statis mengikuti pola `/privasi`/`/syarat-ketentuan`: tujuan wakaf, gambaran enam langkah proses sebagai informasi, dan kanal kontak `/bantuan` — tanpa formulir, tanpa unggahan, tanpa pendaftaran minat (AC12). Syarat umum yang rinci masih dalam proses finalisasi, bukan difabrikasi. Menutup PUB-072 (`docs/product/screen-inventory.md`) dan celah nyata yang ditinggalkan komentar berkas homepage sejak Stage 3 tiket 03 (tautan CTA sekunder Wakaf Tanah yang sebelumnya berupa kontrol nonaktif karena rute ini belum ada).
