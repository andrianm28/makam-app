...
- [x] Marketplace categories and vendor processing pass. — Categories/browse/product-detail/cart/checkout/manual-payment/order-tracking: `tests/browser/e2e-marketplace.spec.ts` (8 `describe` blocks, all green in CI). Vendor-side processing: `tests/browser/e2e-admin-vendor.spec.ts::'vendor can accept an incoming order through the one-click work-queue action'` (added this pass) closes the previously-open half — a vendor can log in, reach their own work queue, and actually accept an order, not just view it. That test has passed a real local CI-equivalent run (Postgres 18 + Redis, the pinned app image, `php artisan serve`, a real Chromium browser) but not yet an actual CI run — this branch has not been opened as a PR yet, so CI confirmation of it specifically is still pending. Vendor transaction history and payout visibility: same file's `'vendor transaction history is reachable and scoped to this vendor only'` / `'vendor payout status/reference is visible and scoped'` (PR #142, CI-green). Two of the three cited tests are CI-confirmed; the new accept-order test is confirmed locally and awaits its first CI run.
