...
**Foundations deferred:** `platform-notifications` · `platform-document-vault` (blocked on OQ-4, resolved 23 Aug 2026 — self-hosted, `ADR-0027`) · `platform-payment-adapter` (blocked on OQ-5 / FIN-DEC) · `platform-financial-ledger` (blocked on FIN-DEC) · full `platform-outbox` (Horizon supervisors, bounded replay, 10k-import isolation).

**Feature specs deferred entirely:** `funeral-marketplace-and-vendor-portal` (cart, checkout, **vendor portal**) · `funeral-case-management` (event shapes for its manager-assignment and task-overdue events are reserved in `docs/contracts/event-catalog.md`'s `funeral_case.*` rows, marked deferred there — no producer exists until this spec starts, per CONTRACT-04 07 Sep 2026) · `at-need-booking` · `package-and-service-bundles` · `cemetery-operator-dashboard` · `recurring-care-subscriptions` · `grave-care-fulfillment` · and all five gated specs (`pre-need-contracting`, `plot-inventory-and-reservation`, `certificates-and-agreements`, `visitation-booking`, `memorial-and-qr`).
...
| `platform-document-vault` | 6 | Blocks booking Step 7; **gated on OQ-4 object storage** (resolved 23 Aug 2026 — self-hosted, `ADR-0027`) |
...
| S2-T7 | Encrypted daily staging backup to remote object storage + **restore test with evidence** | infra | **M-4** | 2 pd | ⚠️ **HUMAN** | ⚠️ **prepared 25 Jul (Batch 2.6C) — script + restore runbook ready; OQ-4 resolved 23 Aug 2026 (self-hosted, not remote/external, `ADR-0027`) — the task's own "remote object storage" premise is superseded, not merely unblocked. Still not executed: `examples/backup-staging.sh` still targets `BACKUP_S3_*` variables and needs rewriting for a self-hosted destination, that destination itself is not yet chosen, and no backup or restore has actually run — see `docs/operations/backup-and-restore-runbook.md`.** |
...
| ~~No remote object storage provisioned~~ — **OQ-4 resolved 23 Aug 2026**: the decision was self-hosted storage, not any external/remote provider (`ADR-0027`) | Med | High | Uploads already run on this adapter in development; extending it to production is a config change (setting `DOCUMENT_VAULT_OBJECT_STORAGE`), not new code — but no production environment exists yet, so nothing runs there today. S2-T7's own backup destination (self-hosted path/volume + `backup-staging.sh` rewrite) is separate, still-open follow-up work — see `docs/operations/backup-and-restore-runbook.md` |
...
**Status note, 20 Aug 2026.** This log stops at 09 Aug 2026 and is not maintained past that date. Batches P1 through P5a shipped 15–16 Aug 2026 (admin order management, admin data management, plot inventory + reservation, memorial/QR/visitation, certificates/agreements/pre-need) and are not recorded here as deployment entries. For current shipped status, read `docs/domain/traceability-matrix.md`'s own revision notes (v0.10 through v0.15), not this table.
...
|---|---|---|---|
| **6** | `platform-notifications` (full) · `platform-document-vault` · `platform-outbox` (Horizon, replay, 10k isolation) | 2 | ~~**OQ-4** object storage~~ resolved 23 Aug 2026 (self-hosted, `ADR-0027`) · **OQ-7** scanner |
...
> **Correction, 11 Aug 2026 (`platform-payment-adapter`, lane `lane/l3-payment-adapter`, Tasks 1-10 of 10 — lane build complete):** `G-PAY-01` and the FIN-DEC approvals in `release-gates.md` §H remain **closed/ungranted** — this row's gating is unchanged. Build progress within the deny-only guard (`GuardPaymentSession` fails closed on every one of its six conditions except the server-resolved mode check, so no `payment_sessions` row can exist): a durable, signature-verified webhook receiver; a self-contained manual-fallback submission/verification slice (`payment_verifications`); a self-contained refund/chargeback recording slice (`payment_reversals`); and a ledger seam contract test against L4's `Journal` interface. All decoupled from the blocked session/journal/provider path by design — none constitutes a working paid path. **Sandbox-exercised evidence: NONE.** Task 8's live SumoPod sandbox smoke test (real API key, real webhook secret, `payment_link_url` resolution) is explicitly **NOT TESTED** — it requires a human or ops-provisioned dev-host run outside an agent session (no outbound `PaymentProvider` integration exists to exercise, and the agent session's own permission system correctly refuses to read the sandbox credentials). Task 9's whole-branch review (3 concurrent slices covering AC1-3/8/9/11, AC4/10/12, AC5/6/7/13/14) closed with 0 Critical/Important findings across the entire branch. CI green (PostgreSQL 18) on every pushed commit through the branch's full history. PR opened against this branch, pending human merge — full itemized disposition of every surfaced gap, deferred minor, and cross-lane reconciliation item lives in `docs/planning/retrofit-backlog.md` §2 and `.kiro/specs/platform-payment-adapter/tasks.md`'s NOT TESTED section.
...
| R-5 | ~~Production managed Postgres provider undecided (N-3)~~ — **resolved 23 Aug 2026**: decided self-managed on the shared host, not a managed provider (`ADR-0027`; N-3) | **High** | **High** | No longer blocks production planning; no PITR is now an accepted production risk, recorded in `ADR-0027`'s single-host section | Product |
| R-6 | No object storage → blocks backups **and** uploads | Med | High | **OQ-4 — resolved 23 Aug 2026: self-hosted, `ADR-0027`.** Uploads already run on this adapter in development; extending it to production is a config change, not new code — but no production environment exists yet, so nothing runs there today. Backup destination still needs a concrete decision + `backup-staging.sh` rewrite — see `docs/operations/backup-and-restore-runbook.md` | Product |
...
| **N-3** | ADR-0021 requires **managed** PostgreSQL with PITR for production; today's setup is a self-hosted Docker container with no backups. **Resolved 23 Aug 2026 — not by choosing a managed provider, but by deciding production stays self-managed on the shared host** (`ADR-0027`'s single-host decision; `ADR-0021` itself superseded, see its own 23 Aug 2026 status update). No longer blocks production planning; no PITR is now an accepted production risk rather than an open finding. | **High** | R-5, Sprint 16 |
...
| **OQ-4** | **S3-compatible object storage — which provider?** Needed for staging backups (S2-T7) **and** later document uploads. Local MinIO on the 2/4 host is explicitly forbidden (this framing is itself superseded — see resolution). | **Resolved 23 Aug 2026 — self-hosted via `App\Platform\DocumentVault\Adapters\LocalFilesystemObjectStorage`, not an external provider** (`ADR-0027`'s "Production graduation — single-host decision" section; the codebase never adopted MinIO specifically, so this is not "MinIO" being un-forbidden, it's local filesystem storage being extended to production). Document uploads already run on this adapter in development; extending it to production is a config change (setting `DOCUMENT_VAULT_OBJECT_STORAGE`), not new code — but no production environment exists yet, so nothing runs there today, and this remains blocked on OQ-7 regardless. S2-T7's own backup-destination follow-up (concrete path/volume, `backup-staging.sh` rewrite) is separate, still-open work — see `docs/operations/backup-and-restore-runbook.md`. | S2-T7, R-6 | **Before Sprint 2** |
...
| **OQ-6** | **Production managed PostgreSQL provider?** (N-3) ADR-0021 requires managed + PITR. | **Resolved 23 Aug 2026 — self-managed PostgreSQL 18 on the shared `yiemvm` host, not a managed provider** (`ADR-0027`'s "Production graduation — single-host decision" section; `ADR-0021`'s original managed-PostgreSQL/PITR decision is superseded — see that ADR's own 23 Aug 2026 status update). No PITR is an accepted production risk under this decision, not an open question. | Sprint 16 | Before Sprint 10 |
| **OQ-7** | **Malware scanner for production?** Always-on ClamAV is forbidden on the combined host; dev may use a deterministic mock. Production needs a real fail-closed scanner. | Mock in dev; production undecided | Sprint 5–6 | Before Sprint 5 |
...
4. The combined host remains non-production only and is never cited as production capacity evidence.
...
| **N-3** | High | 15 | OQ-6 | No managed Postgres provider — resolved 23 Aug 2026, self-managed on the shared host per `ADR-0027` (no PITR is now an accepted production risk, not an open finding) |
