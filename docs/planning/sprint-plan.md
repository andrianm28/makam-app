...
| ~~No remote object storage provisioned~~ — **OQ-4 resolved 23 Aug 2026**: the decision was self-hosted storage, not any external/remote provider (`ADR-0027`) | Med | High | Uploads already unblocked (`LocalFilesystemObjectStorage` in production use); S2-T7's own backup destination (self-hosted path/volume + `backup-staging.sh` rewrite) is separate, still-open follow-up work — see `docs/operations/backup-and-restore-runbook.md` |
...
| R-6 | No object storage → blocks backups **and** uploads | Med | High | **OQ-4 — resolved 23 Aug 2026: self-hosted, `ADR-0027`.** Uploads unblocked (`LocalFilesystemObjectStorage` already in use). Backup destination still needs a concrete decision + `backup-staging.sh` rewrite — see `docs/operations/backup-and-restore-runbook.md` | Product |
