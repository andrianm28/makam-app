...
| **N-12** | Closing N-9 item (3) (Batch 3.5, 26 Jul 2026 — `GateActivationRecorder` now calls `Outbox::record()` for real) surfaced that `docs/contracts/event-catalog.md` has **no entry for a gate-state-change event**. Checked directly against the catalogue's own event table: 25 events listed, closest analogues are `cemetery.capability_changed.v1` and other domain-specific "X changed/confirmed" events, but nothing named anything like `feature_gate.state_changed` or `gate.activated`. `platform-outbox` AC3 ("use event types from `event-catalog.md` and SHALL NOT restate the catalogue") means this gap is not this class's to resolve unilaterally by inventing a catalogue entry. Resolution taken: `GateActivationRecorder` still writes the `outbox_events` row (AC9's actual requirement — "emit an outbox event," not "use a catalogued name"), using the clearly-provisional name `feature_gate.state_changed.v1`, following the catalogue's own `noun.verb_past_tense.vN` convention even though it is not itself a catalogued entry. **What still needs to happen:** whoever owns `event-catalog.md` should add a real `feature_gate.state_changed` (or a deliberately chosen alternative name) row, with Producer = `FeatureGate`/`GateActivationRecorder` and Main consumers = whichever projections/notifications AC9 anticipates ("dependent projections and notifications react") — none of which exist yet in this repo. Until that lands, `feature_gate.state_changed.v1` is a real, uncatalogued event type in production data, not a placeholder that disappears on its own. | Medium | `docs/contracts/event-catalog.md`, `app/Platform/FeatureGate/GateActivationRecorder.php` |
| **N-13** | S4-T2 (`public-faq` backend/domain batch, 26 Jul 2026) needed a schema-owning module for three tables `public-faq`'s own `design.md` names explicitly (`faq_categories`, `faq_articles`, `faq_article_versions`) — `app/Domain/README.md`'s rule is one directory per module boundary listed in `docs/architecture/overview.md` §5. Checked directly against that table: 23 modules listed (`IdentityAccessAdapter` through `FeatureGate`), and **none of them is `Faq`** — a genuine gap between the module-boundary table and a spec that clearly needs to own tables, not a naming mismatch (no existing row's "Responsibility" text covers FAQ content either). Resolution taken: created `app/Domain/Faq/{Models,Actions}` anyway, following the established one-directory-per-module convention despite the missing table row — the alternatives (cramming these three tables into an unrelated existing module's directory, e.g. `NotificationAdapter` or `CemeteryDirectory`, or skipping the directory convention entirely for this one spec) are both worse than the gap this creates, and `app/Domain/README.md` itself already anticipates modules being added over time ("Created empty during the Sprint 1 scaffold... module boundaries are never successfully retrofitted after features exist"). **What still needs to happen:** whoever owns `docs/architecture/overview.md` next should add a `Faq` row to §5's module table (suggested Responsibility text: "Public FAQ categories, articles, versioning, and publish lifecycle"), so the table and the actual `app/Domain/` tree agree again. Not resolved here — adding a row to a document this batch does not own is a documentation-ownership decision, not an implementation one, the same reasoning N-7's Pre-Need status-enum gap and N-12's event-catalogue gap already applied to their own out-of-scope document edits. | Medium | `docs/architecture/overview.md` §5, `app/Domain/Faq/**`, `app/Domain/README.md` |
| **N-14** | S4-T2's public FAQ Livewire batch (26 Jul 2026) was this repo's first-ever use of ANY `<x-mk.*>` design-system primitive rendered through a real Livewire full-page component (`resources/views/livewire/**` did not exist before this batch). Every `<x-mk.button>` usage on `/faq` and `/faq/{articleSlug}` — five call sites across two files, only one of which even used a `wire:loading` directive — failed the first real CI run with `ErrorException: Undefined variable $loading`, the identical message Batch 3.2's `GateClosedBladeComponentsRenderTest` had attributed to a narrower `Blade::render()`-with-nested-named-slots limitation; that earlier diagnosis is now known to be incomplete, since none of these five call sites use `Blade::render()` or a named slot. Traced Blade's real compiled `@props` output (`Illuminate\View\Compilers\Concerns\CompilesComponents::compileProps()`) directly against the installed source used elsewhere in this project for exactly this kind of verification — every prop assignment in the compiled preamble uses `??`, which does not throw for an undefined variable, so the failure point was not conclusively pinned down before this was fixed. `<x-mk.badge>` (no `loading`-named prop) renders correctly on the same pages in the same render tree, so the trigger looks specific to the prop name `loading` — plausibly a collision with Livewire's own `wire:loading` feature somewhere in its Blade-compiler extension (`Livewire\Mechanisms\ExtendBlade\ExtendedCompilerEngine::evaluatePath()`, which replaces Laravel's default view-evaluation closure with one bound to the Livewire component instance) — but this is not confirmed with certainty. **Fix applied**: every affected call site now hand-writes the button markup, reproducing `button.blade.php`'s own already-approved literal recipe directly, the same defensive pattern already used elsewhere in this batch for a `<x-mk.field>` limitation. This turned out to include `<x-mk.header>` itself (`resources/views/components/mk/header.blade.php`, Batch 2's own shared navigation primitive, not owned by this batch) — its two "Bantuan" buttons render on every page via `layouts/app.blade.php`, so fixing only the FAQ views' own buttons left every page still broken; both were hand-written the same way. `resources/views/components/mk/table.blade.php` has one more real (not doc-comment) `<x-mk.button>` usage that is currently unreachable (no Livewire view uses `<x-mk.table>` yet) — left as-is since fixing unreachable code is unnecessary scope, but it will hit this identical bug the moment a future batch renders it from a Livewire component; fix it at that point, not preemptively. **What still needs to happen**: root-cause this properly (ideally with a minimal reproduction against the installed Filament/Livewire source already available on this host) and either fix `button.blade.php` itself or file an upstream Livewire issue if it is a real package bug — until then, no future batch should use `<x-mk.button>` inside any `resources/views/livewire/**` view, nor inside any other `<x-mk.*>` component that might get rendered from one, without re-verifying this is actually fixed. | High | `resources/views/livewire/public/faq/*.blade.php`, `resources/views/components/mk/header.blade.php`, `resources/views/components/mk/button.blade.php`, `resources/views/components/mk/table.blade.php`, any future Livewire full-page component |
