...
| S1-T5 | Fix Postgres volume path in repo compose example | docs | **H-1** | 0.5 pd | — | ❌ **open** |
| S1-T6 | Laravel 13 scaffold + all 8 baseline artefacts; establish `overview.md` §5 module namespaces | all | **H-4** | 4 pd | — | ❌ **open — master blocker** |
...
| S1-T8 | CI pipeline skeleton (GitHub Actions) | all | H-4 | 2 pd | — | ❌ open |
| S1-T9 | `.gitignore` + `.claude/settings.json` hardening; commit design-system files | repo | **M-2, M-3** | 0.5 pd | — | ⚠️ partial |
| S1-T10 | ADR-0028 (design system) + ADR-0029 (scaffold) + ADR-0030 (platform foundation specs) | all | L-6 | 1 pd | — | ❌ open |

**Total: ~12 pd · 4 of 10 done (the C-2 chain, executed 25 Jul 2026)**
