...
| S4-T4 | Booking wizard shell Steps **1–5** + autosave/resume across sessions | `public-booking-wizard` AC1–AC6, AC11–AC13 | identity, feature-gate, audit | 5 pd | — | 🔵 **Implemented, pending merge/CI** (09 Aug 2026) — resumed 08 Aug after the 26 Jul pause. Steps 1–5 built on branch `docs/design-system-and-planning` (worktree `booking-wizard-steps-1-5`), whole-branch reviewed, review findings fixed, booking test surface green locally (109 tests). **Not merged, not CI-verified, not deployed** — this row is not `✅ Done` in the same sense as the deployed rows above. AC13's "unskippable" half and the real idempotency/version wiring landed only in the final fix wave; see the plan's own AC13 correction note |
