...
- `ServiceComplaintInfolist`: full complaint text, filing customer, linked `WorkOrder`/care-plan context, resolution notes, and (when set) a link to the resulting `MakeGoodOrder`. Add `ServiceComplaint::customer(): BelongsTo` (`User::class, 'customer_id'`) to the model for the same reason as `workOrder()` above. Note: the migration declares `customer_id` as `foreignUuid` while the model casts it `integer` and `FileComplaint` passes a plain `int` — `users.id` is a bigint PK, so the migration's `foreignUuid` declaration is the actual latent bug here (the FK constraint likely never validates correctly against `users.id`), not the model or `FileComplaint`. **Do not silently change the column type as a side effect of this work** — flag it in the task's own report as a real, pre-existing, separate defect worth its own investigation; this spec only adds the `belongsTo()` relation on top of the existing (buggy) column, it does not fix the column.
...
- The `service_complaints.customer_id` foreignUuid-vs-integer type mismatch found during research — real, pre-existing, but unrelated to this work; flag for a future, separate investigation.
