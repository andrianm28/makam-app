...
1. **PostgreSQL 18 — NOT TESTED.** `phpunit.xml` pins `DB_CONNECTION=sqlite`, `DB_DATABASE=:memory:`, so every run above was SQLite. Plan §5 requires a real PostgreSQL 18 run before done, because this changes authorization and audit behaviour. Not executed here: the host is under memory pressure with four other lanes active, and the instruction was scoped runs only. **This is an outstanding plan requirement, not a waived one.**
   ...
   **RESOLVED 12 Aug 2026 (review nit N-6) — original text above kept unedited so the history stays honest.** The lane driver has since run the payment module suite green on real PostgreSQL 18.4 in a disposable container: 218 tests, 928 assertions. The plan §5 requirement is met; this line is no longer an open gate. Note the run predates the SF-6 fix round, whose new refusal-audit assertions were exercised on SQLite only — see the fix-round report's own NOT TESTED section.

   **NARROWED 12 Aug 2026 (whole-branch review finding SF-3) — the paragraph above over-claimed and is corrected here rather than rewritten.** "The plan §5 requirement is met; this line is no longer an open gate" was true **only for `8be7e49`**. The 218-test run predates commit `51d6a85`, which added the SF-6 refusal-audit write and its assertions, so it could not have covered them. The gate was therefore still open at `51d6a85` when that sentence was written. It has since been closed on the evidence recorded in the fix-round report's §8 items 1 and 2: the lane driver ran the payment module suite green on real PostgreSQL 18.4 at `51d6a85`'s content — 227 tests, 1028 assertions. Read those two entries, not this one, for the current state of the PostgreSQL and full-suite gates.
...
1. **The instructed doc-block sentence was factually wrong and was NOT written.** The brief asked the authorizer's class doc block to record "that it fails closed today because `ActorContext::$roles` is `[]` under the current local identity adapter." That is stale. `LocalUsersTableIdentityAccessAdapter` resolves real, live grants through `Roles\ActorRoleReader` (commit `77532bc`, `feat(identity-seam): resolve real roles and scopes into ActorContext`, an ancestor of this branch), and its own doc block says so explicitly. I detected this while reading the adapter and wrote the doc block accurately; lane `lane-payment-auth-hotfix` independently sent the same correction mid-task, which I applied to tighten the wording further. The authorizer now states it is live enforcement, names both stale sources (plan §6 and the four `FinancialLedger` authorizers) as deliberately not repeated, and keeps the still-true framing that an empty role list means "holds no grants today", never "no role required".
   ...
   **Correction, 12 Aug 2026 (review finding SF-3).** This item originally closed "Plan §6 is therefore stale on this point and should be corrected by whoever owns the plan doc." That correction had **already landed** when this was written: commit `f548b53` ("docs(payment): correct a stale claim about the role seam being unwired") rewrote §6 into "Impact — this fix is live, not dormant" and is an ancestor of the implementation commit `8be7e49`. Plan §6 is accurate and safe to follow; nothing is owed here. The authorizer's class doc block, which named plan §6 as a stale source, was corrected in the same fix round. The still-true half of the original claim — that the four `FinancialLedger` authorizer doc blocks carry it — is unchanged and remains out of scope (item 2 below).
...
3. **Authorization placed above the `reversalType` `match`** in `RecordPaymentReversalController`, not below it. The `match`'s `default` arm is unreachable behind the route's `->whereIn('reversalType', ['refund', 'chargeback'])` constraint, so this is not load-bearing; putting authorization unconditionally first is simply the ordering that cannot be got wrong later.
