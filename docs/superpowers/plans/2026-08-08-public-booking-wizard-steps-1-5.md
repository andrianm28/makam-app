...
- **R5 (Task 9 degradation test ordering — execution finding, 09 Aug 2026):** Task 9's `test_a_failed_cemetery_read_degrades_honestly_instead_of_500ing` drops the cemetery tables and THEN calls `saveStep1()`. That cannot pass: `booking_drafts` FK-references `cemetery_packages` and `cemeteries`, so on SQLite the step-1 INSERT fails with `no such table: main.cemetery_packages` (verified), and on Postgres the `DROP TABLE cemetery_packages` itself is blocked by the FK dependency. The test's intent is render-time degradation, not a write against a dead directory. Corrected implementation: create the draft with the schema intact, drop the four directory tables (same list as `RenewalStartTest`'s degradation test), then resume the draft via `/pemesanan-makam/draft/{draftId}` and assert OK — the resume render hits `CemeteryPublicQuery::inCity()` failure and must degrade (unavailable alert), never a 500.
- **R6 (post-review follow-up, 09 Aug 2026):** the batch passed the local SQLite suite but the degradation test and Step 2 markup needed cross-engine/accessibility corrections, caught in human review.
  - **`Schema::disableForeignKeyConstraints()` cannot unblock the Postgres drops.** It compiles to `SET CONSTRAINTS ALL DEFERRED`, which only affects DEFERRABLE constraints; Laravel creates these FKs as NOT DEFERRABLE. Verified on Postgres 18: `SET CONSTRAINTS ALL DEFERRED` + `DROP TABLE` still raises `2BP01` (2BP01 also ignores the FKs' `nullOnDelete`, which only applies to row DELETEs). Corrected: `BookingWizardRouteTest` now drops the two `booking_drafts` FKs via `Schema::table('booking_drafts', ... dropForeign([...]))` before the table drops — engine-neutral (SQLite rebuilds the table, Postgres runs `ALTER TABLE ... DROP CONSTRAINT`), and the draft row survives because the drops happen after `saveStep1()`.
  - **Same FK broke three pre-existing degradation tests on Postgres**, which CI caught: `RenewalStartTest::test_a_failed_cemetery_read_degrades_honestly_instead_of_500ing`, `CemeteryDirectoryIndexRouteTest::test_the_page_survives_the_cemeteries_table_being_unreadable`, and `CemeteryDetailRouteTest::test_a_package_read_failure_is_reported_separately_from_an_empty_package_list`. Each got one line — `Schema::dropIfExists('booking_drafts')` (empty in those tests) at the top of its drop list — so the FK tripwire still guards any *future* table, per those tests' documented intent.
  - **Step 2 cemetery cards were not keyboard-focusable.** `<x-mk.card as="button">` silently renders a `<div>` (the card component only promotes to `<a>` when an `href` is present), so the clickable "cards" were unreachable by keyboard. Replaced with a real `<button type="button">` styled to match the card look (border/bg/hover/shadow + `focus-visible` ring), preserving the visual and the `wire:click`.
  - **Committed `2026_08_08_130000_create_booking_drafts_table.php` failed `pint --test`** (`fully_qualified_strict_types`, `ordered_imports`, etc.). Reformatted by `vendor/bin/pint`; cosmetic only.
  - **Verified:** full suite green on the CI engine — Postgres 813 passed / 0 failed; SQLite 791 passed with only the two pre-existing acknowledged `CASCADE`-on-SQLite quirks (`HomePageRouteTest` faq-highlights, `EloquentGateRegistrySourceTest` registry-load), both green on Postgres.
...
**Spec coverage.** `public-booking-wizard` AC1 (nine-step shell, Task 9/2), AC2 (five cities, Task 9), AC3 (Step 2 fields, Task 6/9 — photo/facilities/price rendering deferred to whoever finalizes the Blade markup per Task 9 Step 6's note, since this plan's author did not verify every `<x-mk.card>` prop against the live component), AC4 (Step 3, Task 6/9), AC5 (Step 4, Task 7/10), AC6 (Step 5 quote lines/total, Task 4/10 — as a computed presentation, not a Quote row, per Global Constraints), AC11/AC12 (autosave/resume/back-nav, Task 9/10/11), AC13 (idempotent/versioned/unskippable, Task 6/8 — **see the correction below**) are all covered. AC7-AC10, AC14, AC15 (Steps 6-9, notifications) are explicitly out of scope. `booking-and-order-orchestration` AC2 (Task 3/6/8), AC3 (Task 6/7) are covered; AC1, AC4-AC14 are explicitly out of scope or partially addressed only insofar as Steps 1-5 need them (AC1's "shared entry point" is Task 9's routes).

**AC13 correction, 09 Aug 2026 (final whole-branch review, finding I1/I2).** The claim above was accurate for two of AC13's three words and false for the third, and two of the "covered" halves were covered only on paper:

- *Idempotent* — the Action's replay branch was real and tested, but **unreachable from the UI**: every `SaveBookingDraftStep` call site in `BookingWizard` generated a fresh `Str::uuid()`, so `last_idempotency_key` could never match. Fixed — the key is now derived from `(draftId, step, payload)`, and `BookingWizardSaveIntegrityTest` proves a repeated identical save does not write again.
- *Versioned* — `?int $expectedVersion` existed (Task 8) but **no caller ever passed it**, so no conflict was detectable in practice. Fixed — the component holds `$version` and passes it on every save, and handles `BookingDraftVersionConflictException` by re-hydrating and surfacing an inline error.
- *Unskippable* — **was not implemented at all.** Task 6/8 delivered boundary rejection (`assertKnown`, `> LAST_IMPLEMENTED`), which is a different rule; nothing stopped step 3 being saved on a draft that had never completed step 1, and `$currentStep`/`$completedSteps` were client-writable Livewire properties. Fixed — `SaveBookingDraftStep::validateStepSequencing()` rejects step N when step N-1 is absent from the draft's own `completed_steps`, and the component's step state is `#[Locked]`.

As of that fix wave AC13 is covered as: idempotent (derived keys, replay-tested) + versioned (expected-version passed and conflict-handled) + boundary rejection + full step sequencing. Evidence: `tests/Feature/Domain/Booking/Actions/SaveBookingDraftStepIdempotencyTest.php`, `tests/Feature/Domain/Booking/Actions/SaveBookingDraftStepTest.php` (§Step sequencing), `tests/Feature/Livewire/Public/Booking/BookingWizardSaveIntegrityTest.php`.
