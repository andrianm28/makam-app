...
- **Corrected 4 Sep 2026:** this constraint described `service_complaints.customer_id`'s `foreignUuid`-vs-`integer` type mismatch as a real, pre-existing defect that Task 1 must not touch. That was stale even when this plan was written — `database/migrations/2026_08_22_100000_fix_customer_and_uploader_identity_columns.php`, merged 13 days earlier, had already redeclared the column as `foreignId('customer_id')->constrained('users')`, giving it a real bigint type and a real FK to `users.id`, matching the model's `integer` cast. There was no live defect for any task to avoid fixing; this line's premise was wrong from the start.
