...
**SECOND CORRECTION, 09 Aug 2026 — the pairing is a THREE-way group, not a two-way pair.** The correction above was written when only two stubs were known. Lane 1's scope-honesty reviewer then found a third, `BookingWizardComingSoon.php`, and **no lane in this wave owns Booking**, so that stub is orphaned across all four lanes rather than covered by anyone's retrofit. Independently verified in this worktree before amending: `app/Livewire/Public/ComingSoon/` contains exactly three files (`BookingWizardComingSoon.php`, `MarketplaceComingSoon.php`, `RenewalComingSoon.php`); all three return `view('livewire.public.coming-soon', …)` (`BookingWizardComingSoon.php:25`, `MarketplaceComingSoon.php:26`, `RenewalComingSoon.php:24`), so they share one view; and `routes/web.php:55-58` already names all three together as *"now dead code, deliberately left in place rather than deleted in this same change."* Per this repository's append-correction convention the superseded two-way text is left as written above.

**Binding instruction for Task 3:** do **not** delete `MarketplaceComingSoon.php`, and do **not** touch `routes/web.php` or `BookingWizard.php` at all. The stub is disposed as a **ledgered gap** in Task 5, recorded as a combined future cleanup item worded *"`MarketplaceComingSoon` is one of three orphaned ComingSoon stubs (with `RenewalComingSoon` and `BookingWizardComingSoon`) sharing `coming-soon.blade.php` — one future PR should remove all three together, not a two-way pairing."* No lane deletes any of the three stubs this wave. The Task 1 schema/dead-code slice still gathers the full reference-grep evidence — it now feeds the ledger entry instead of authorising a deletion.
...
- [ ] **Step 1: Give every self-flagged gap an explicit disposition** in a new `retrofit-backlog.md` §2 subsection headed `### funeral-marketplace-and-vendor-portal (Marketplace browse skeleton), retrofitted 09 Aug 2026`, matching items 1-4's existing two-column `| Gap | Disposition | Reason |` table format exactly. One row each for: AC2's missing columns; the category-code OPEN QUESTION; the `{productSlug}` IA drift; §6.1/§6.6/§6.8 unimplemented states; accessibility geometry; `tasks.md` §NOT TESTED bullets 2-5; `VendorProcessingStatus`'s forward definition; **the `MarketplaceComingSoon` dead stub — worded "`MarketplaceComingSoon` is one of three orphaned ComingSoon stubs (with `RenewalComingSoon` and `BookingWizardComingSoon`) sharing `coming-soon.blade.php` — one future PR should remove all three together, not a two-way pairing," carrying the Task 1 grep evidence that it is unreferenced, and noting that `BookingWizardComingSoon` is owned by no lane in this wave;** **and the `mvp-scope`/`sprint-plan` cart-checkout contradiction, carrying this plan's OPEN DEPENDENCY text including the "second retrofit must not start" sentence.**
...
- [ ] `MarketplaceComingSoon.php` is **still on disk, untouched**, and ledgered in `retrofit-backlog.md` §2 as a combined future cleanup covering **all three** orphaned stubs (`MarketplaceComingSoon`, `RenewalComingSoon`, `BookingWizardComingSoon`) that share `coming-soon.blade.php`. `routes/web.php` and `BookingWizard.php` have zero changes in this branch's diff.
