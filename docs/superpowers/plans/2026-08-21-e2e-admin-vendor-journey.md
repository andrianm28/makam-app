...
- **Corrected during Task 1 (see the SDD ledger for the full ruling):** the seed migration must be gated behind a dedicated, default-false config flag (`config('e2e_fixtures.seed_admin_vendor_users')`, backed by `env('SEED_E2E_ADMIN_VENDOR_USERS', false)`, new file `config/e2e_fixtures.php`) — writing real rows into `actor_role_assignments`/`scope_assignments`/`audit_events` unconditionally breaks 8+ pre-existing tests that assert exact/zero counts on those tables, since `RefreshDatabase` applies every migration once per PHPUnit process. This is the same fix shape already used for `THROTTLE_PUBLIC_GUEST_DISABLED` (`config/rate_limiting.php`) after a bare `environment('testing')` gate was tried and rejected for the identical collision-with-phpunit.xml reason. `.github/workflows/ci.yml`'s browser-test job's "Migrate the application database" step DOES need one new env var, `SEED_E2E_ADMIN_VENDOR_USERS: "true"`, alongside its three existing CI-step-scoped overrides — this replaces the incorrect "no new CI workflow step is needed" constraint originally written here.
