...
- **design.md's own "Open decisions"**: AC7 ("FAQ content must reflect active payment/Urgent gates, never an unsupported claim") has **no enforcement mechanism** — "no FAQ code path resolves a gate today (grep... finds no `FeatureGateResolver`/`ModeResolver` call), and no test references AC7. AC7 therefore rests on editorial review." This is a real, self-flagged gap this retrofit's whole-module review must give an explicit disposition to — is it ledgerable as "editorial review is an acceptable enforcement for this AC" or does it need a real code guard? Task 2's whole-module reviewer rules on this with reasoning, doesn't silently inherit the design doc's framing.
...
design.md's own text: "no FAQ code path resolves a gate today... AC7 therefore rests on editorial review." Does this stay as an accepted, documented posture (ledgered), or is it a real Important finding needing a code-level guard within this retrofit's bounded fix wave? Reason it through — don't just inherit design.md's framing without independent judgment, and don't invent gate-integration code that duplicates the `FeatureGate` module's own established resolver pattern without checking whether that's actually warranted by AC7's text.
