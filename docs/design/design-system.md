...
| `labels` | **legacy fallback — the OLD nine-step booking wording, see below** | Ordered step labels for the journey being rendered |

**Correction, 3 Sep 2026 (post-merge): the component's hardcoded `labels` prop default is NOT the four labels above.** It is still the pre-step-reduction nine-step booking wording (`Lokasi · TPU/TPS · Jenis Layanan · Pilih Layanan · Ringkasan · Data Pemesan · Data Almarhum + Dokumen · Pembayaran · Konfirmasi`), left in the component unchanged, as a safety net against an accidental reword for any future caller that omits the prop entirely — not because it is meant to be reached. It is not reached by anything in the app today: booking's own Blade view (`resources/views/livewire/public/booking/wizard.blade.php`) explicitly passes `:labels="\App\Domain\Booking\BookingWizardScreen::labels()"` (the four labels quoted at the top of this section), and renewal explicitly passes `RenewalWizardScreen::labels()` — no real call site relies on the component's own default. `App\Domain\Booking\BookingWizardScreen` is the canonical source for booking's four screen labels, not this prop's default value; do not read the default as documentation of what booking renders. This is a component-contract change, not a token change, so [§9.4](#94-changing-a-token) does not apply and **no ADR is required**; §9.4 governs `tokens.css` only.
...
**As shipped, the 5-item bottom navigation** (Beranda, Pemesanan, Perpanjangan, Akun, Bantuan) renders on mobile (`lg:hidden`) with fixed positioning (`fixed inset-x-0 bottom-0`), a 5-column grid layout, and a height set by `h-[var(--mk-bottomnav-total)]` with safe-area padding `pb-[var(--mk-safe-bottom)]`. The background is `bg-neutral-0` with a top border `border-t border-neutral-200`. Each tab is a full-height, full-width link with centered flex layout (`flex-col items-center justify-center`), icon (1.25 rem / 5 px size, shrink-proof), text label (`text-2xs`), and a small gap. **Active marking:** `text-primary-700 border-t-2 border-primary-600` (2 px top border override in primary color, replacing the default transparent top border). **Inactive:** `text-neutral-700 border-t-2 border-transparent`. **Accessibility:** `<nav aria-label="Navigasi utama">` and `aria-current="page"` on the active tab. **Touch targets:** all dimensions meet or exceed the 44 px WCAG minimum in both height (grid cell height, full `--mk-bottomnav-h`) and width (1/5 of viewport). **Responsive:** Only visible below `lg` breakpoint; the desktop header (§3.10) and its hamburger remain the primary navigation on wider viewports.
