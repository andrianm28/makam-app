...
| `funeral_case.created.v1` | FuneralCase | Operations, notification | At-Need accepted | Live |
| `funeral_case.manager_assigned.v1` | FuneralCase | Operations, audit | Includes handover reason when changed | Not yet produced — owned by `funeral-case-management` |
| `funeral_case.task_overdue.v1` | FuneralCase | Escalation | Idempotent per task/window | Not yet produced — owned by `funeral-case-management` |
...
| `renewal.marked_external.v1` | Renewal | Billing guard | Prevents duplicate period | Not yet produced as of this catalogue version — `MarkExternalRenewal`/`MarkRenewalPaidExternally` have no `Outbox::record()` call yet; a producer pair is in flight on PR #252 |
