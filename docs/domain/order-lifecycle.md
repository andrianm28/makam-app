...
- **No pay-first status is yet eligible for an order-settlement certificate, and none may settle a Pre-Need case.** Four gates still test for `DIBAYAR` exactly — `AgreementCertificate\CertificateEligibilityPolicy::orderSettlementRule()`, `Filament\...\CreateCertificateAction::eligibleOrders()`, `PreNeed\Actions\SettlePreNeed`, and `Filament\...\ViewPreNeedCase::orderIsPaid()` — so once a producer writes the pay-first statuses, all four fail closed. This is a **pending product decision, not an oversight**: settlement asks "was this order settled", which is narrower than `OrderStatus::isPaidOrLater()`'s "has money arrived". That predicate is `true` for `DITOLAK_SETELAH_BAYAR`, so converting these four to it would issue a settlement certificate for an order that was refused and whose money is owed back — and arguably also for `DIBAYAR_MENUNGGU_KONFIRMASI`, which no admin has accepted yet. Deciding which statuses may carry a settlement certificate is the product call these four are waiting on; failing closed is the direction to wait in.
