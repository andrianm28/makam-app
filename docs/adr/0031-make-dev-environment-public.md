...
- Any secret, debug endpoint, or verbose error page unintentionally left reachable on dev is now exposed to the public internet, not just to allowlisted/authenticated parties. **UPDATED 7 Sep 2026 (OBS-08):** this stopped being a "should be reviewed" caveat and became a hard requirement the moment the Laravel app actually landed — `APP_DEBUG=false` is REQUIRED on `dev.makam.co.id` (and on any other publicly reachable vhost this host serves) precisely because this ADR removed the access restriction that used to be the only thing standing between an accidental debug page and the public internet. Before this update, `docs/operations/ai-agent-dev-stg-setup-prompt.md`'s own documented development baseline set `APP_DEBUG=true`, asserting the INSECURE posture as the intended one for a host this ADR had already made public — that document is corrected in the same change. `ci/verify-infra.sh` GATE I12 checks the running containers' effective `config('app.debug')` (via `php artisan about --json`, a non-secret introspection command) and fails when debug is on for any publicly reachable vhost; a human must run it for real on the deployment host to confirm current state, since this script cannot execute there from this environment.
