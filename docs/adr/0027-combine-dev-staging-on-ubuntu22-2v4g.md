...
**Correction (7 Sep 2026, VAULT-05, Phase 3 Batch M9): "implements the full `ObjectStorage` contract" is a functional claim only, not a security one — it does NOT mean encryption-at-rest exists.** Read at face value, the sentence above could be (mis)read as implying the storage layer is production-hardened end to end. It is not: `LocalFilesystemObjectStorage::put()`/`copy()`/`read()` write and read plain bytes directly on the host filesystem — see that class's own `put()`/`copy()` at `app/Platform/DocumentVault/Adapters/LocalFilesystemObjectStorage.php:29` — with NO encryption layer of any kind, application-level or infrastructure-level, at either `storage/app/private/documents` (quarantine + accepted restricted documents: KTP, KK, death certificates) or the equivalent path once this decision routes production traffic here. This was a silent gap, not a reviewed and accepted one, until this correction. It is now tracked explicitly as **OQ-8** below, alongside OQ-7.
...
**OQ-8 (7 Sep 2026, VAULT-05, Phase 3 Batch M9)**: **encryption-at-rest for restricted vault documents does not exist and has no chosen design.** Real Indonesian citizens' identity documents (KTP/KK) and death certificates are stored as plain, unencrypted bytes on this host's filesystem today — see the correction note above this decision. Two real options exist and NEITHER has been chosen (see `docs/superpowers/plans/2026-09-07-batchm9-document-vault-security.md`'s VAULT-05 section for the full options analysis this batch produced, and the `.kiro/specs/platform-document-vault/requirements.md` AC10 note this same batch added):

1. **Application-level envelope encryption** inside `LocalFilesystemObjectStorage` (encrypt on `put()`/`copy()`, decrypt on `read()`), with the encryption key held OUTSIDE `APP_KEY` in dedicated secret management, rotatable independently of it.
2. **Full-disk/volume encryption** for the `storage/app/private/documents` mount at the infrastructure level (e.g. LUKS), transparent to the application layer entirely.

This is a security-architecture decision requiring human sign-off per `AGENTS.md` §Infrastructure-agent execution (key management design is explicitly named there as reserved for human review) — it is NOT resolved by this ADR, this correction, or the batch that authored it. Do not assume restricted documents are encrypted at rest in production until OQ-8 is resolved by an explicit decision and, for option 1, a real implementation.
