...
- **Scope assignment + mandatory query scopes:** `ScopeAssignment` + `ScopeAssignmentResolver`/`ScopeAssignmentReader` back `scope_assignments` grants, consumed today via the "explicit builders" enforcement path design.md's Enforcement points §3 names — `DocumentVault\Policies\DocumentAccessPolicy`, `Notification\RecipientResolver`, `Notification\InAppNotificationInboxQuery`. **Corrected 7 Sep 2026 (STAT-04, batch M5c):** this bullet previously also named `ScopeAssignmentGlobalScope`/`HasScopeAssignments` (the other enforcement path design.md's §3 offers — "global scopes") and claimed cemetery/vendor/order scoping was applied to "every domain query." That was never true: no domain model ever attached `HasScopeAssignments`, so that global-scope path had zero production adopters. Both files were deleted rather than adopted now — attaching closed-by-default query scoping to real cemetery/vendor/order/case/grave/business-entity models with no existing grant backfill would make those models invisible to every actor everywhere, a change judged too large to absorb safely in that batch. See `docs/superpowers/plans/2026-09-07-batchm5c-dead-code-and-static-analysis.md` for the full reasoning. AC5's "enforce record scope at query level" is satisfied today only where a consumer above explicitly applies it — it is not a blanket guarantee across every model.
