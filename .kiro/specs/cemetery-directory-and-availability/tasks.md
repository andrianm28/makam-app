...
- [x] Add capability profile schema and validation. _Requirements: 4_ — done 26 Jul 2026 (Sprint 4 S4-T1, master-data batch): `cemetery_capability_profiles` schema + closed-list validation for all six modes, CI green. **AC7's enforcement is real and code-owned** (recorded 13 Aug 2026): `App\Domain\CemeteryCapability\Models\CemeteryCapabilityProfile::assertValidModeCombination()` throws `InvalidArgumentException` unless `SPECIFIC_PLOT` availability is paired with an `AUTHORITATIVE` registry mode (its doc block cites requirements.md AC7; `CemeteryCapabilityModeCombinationTest` pins the refusal) — the combination rule is no longer an open question; what remains unimplemented is the plot-source adapter itself (task line below), not the rule.
...
- [ ] Converge `cemeteries.facilities` on `docs/product/facility-catalog.md` codes (one-off label mapping, unmapped labels display-only) and add the facility filter to the directory. _Requirements: 13_ — not started (PRD amendment 19 Sep 2026, follow-up order 2)
- [ ] Gate the price and availability filters on sourced tariff and availability mode; keep data-less cemeteries listed with an honest label. _Requirements: 14_ — not started (PRD amendment 19 Sep 2026, follow-up order 2)
- [ ] Add operator contact phone, operating-hours text, and required-document override to the cemetery admin form with the documented fallbacks. _Requirements: 15_ — not started (PRD amendment 19 Sep 2026, follow-up order 1)
- [ ] Derive the `terverifikasi` badge from the active capability profile (`evidence`, `owner`, past `effective_at`); no negative badge. _Requirements: 16_ — not started (PRD amendment 19 Sep 2026, follow-up order 4)
