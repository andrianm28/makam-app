...
- [x] Provide the single shared public entry point/route the nine-step workflow renders behind, for every product type. _Requirements: 1_ — shipped with lane L6 (13 Aug 2026): `/pemesanan-makam` renders `App\Livewire\Public\Booking\BookingWizard` for every product type, with `/pemesanan-makam/draft/{draftId}` for resume (`routes/web.php`; `BookingWizardRouteTest`).
- [x] Enforce `booking-wizard-fields.md`'s per-step field rules server-side on step completion. _Requirements: 3_ — shipped: `App\Domain\Booking\Actions\SaveBookingDraftStep` validates each step's closed lists and field rules server-side (`BookingDraftClosedListValidationTest`, `BookingWizardSaveIntegrityTest::test_a_skipped_step_is_rejected_server_side_even_for_a_caller_that_bypasses_the_ui`).
- [x] Add product-type router. _Requirements: 4_ — shipped: `App\Domain\OrderWorkflow\Actions\SubmitBookingDraft` routes submissions by `ProductType` (`SubmitBookingDraftTest`).
- [x] Link submissions to FuneralCase/PreNeedCase. _Requirements: 5_ — shipped: `SubmitBookingDraft` opens a FuneralCase for `AT_NEED_SERVICE_ORDER` and registers a Pre-Need interest for `PRE_NEED_PLOT_PURCHASE`, stamping `orders.funeral_case_id`/`pre_need_case_id` (`SubmitBookingDraftTest`).
- [x] Generalize confirmation guard for manual or reservation evidence. _Requirements: 6, 7_ — shipped: `App\Domain\OrderWorkflow\Actions\AuthorizeOrderPaymentOpening` resolves actor role and ORDER scope grant (`OrderPaymentOpeningNotAuthorisedException` on denial).
- [x] Preserve immutable quote/version acceptance. _Requirements: 8_ — shipped: `IssueQuote`/`AcceptQuote` + immutable `Quote`/`QuoteLine` versioning (`App\Domain\Quotation\`, `tests/Feature/Quotation/`).
- [x] Enforce the commercial-transition state machine as forward-only, kept structurally separate from case/work/certificate states. _Requirements: 11_ — both halves are now real. The render-separation half: `App\Support\Design\StatusIntent`'s order-lifecycle family (tested, `tests/Unit/Support/Design/StatusIntentTest.php`) keeps e.g. `DIBAYAR`/`SELESAI` as distinct intents rather than one merged "done" badge (see "Order lifecycle → intent" table above). The forward-only transition-enforcement half: `App\Domain\OrderWorkflow\Actions\RecordOrderStatusChange` is the sole writer of `order_status_events` and `orders.status`, rejecting backward/skipped edges via `App\Domain\OrderWorkflow\Exceptions\IllegalOrderTransitionException` against `OrderTransition`'s forward-only graph (shipped with lane L6, 13 Aug 2026; `tests/Unit/Domain/OrderWorkflow/OrderTransitionTest.php`, `tests/Feature/OrderWorkflow/RecordOrderStatusChangeTest.php`).
- [x] Add payment and document security tests. _Requirements: 9, 10_ — shipped: `ApplyPaidEffectsTest` (amount mismatch, currency, exactly-once, never-from-browser-return), `OrderDocumentTest` (AttachOrderDocument security), `RecordOrderStatusChangeTest`.
- [x] Preserve the manual admin/case-manager fallback while an operator has not responded (the "Operator silence" state, above). _Requirements: 12_ — shipped: `MENUNGGU_KETERSEDIAAN` is a first-class state on the forward-only graph (`OrderStatus`, `OrderTransition`), keeping the admin/manual fallback actionable rather than blocking.
- [x] Build the Step 9 order-state read model — reference, status (via `StatusIntent`), invoice state, channel-delivery state, next action, support reference. _Requirements: 13_ — shipped: `App\Domain\OrderWorkflow\OrderReadModel` (`OrderReadModelTest`, 8 tests). **Caveat:** Step 9's confirmation surface currently renders draft-derived confirmation data (`BookingWizard::$confirmationData`), not `OrderReadModel` — the read model is built and tested but not yet the thing Step 9 renders; wiring it in is a presentation-layer follow-up (`public-booking-wizard`).
...
- [x] Implement `StatusIntent` as the single status → intent + icon + label resolver (§3.7), consumed by public, admin, and vendor surfaces. — shipped for the order-lifecycle family: `App\Support\Design\StatusIntent` (`tests/Unit/Support/Design/StatusIntentTest.php`), consumed by the public wizard and admin tables. **The vendor surface half is NOT yet resolved through it**: the L10 vendor panel renders vendor statuses through `VendorOrderStatusPresenter`'s own badge-colour mapping, not `StatusIntent` (see `funeral-marketplace-and-vendor-portal/tasks.md`).
- [x] Expose gated-fallback mode values from the server for UI consumption (§6.9); no front-end flags. — shipped: `App\Platform\FeatureGate\ModeResolver` supplies `PaymentMode`/`WhatsAppMode`/`PreNeedMode`/`GraveSearchMode` to Step 8 and the confirmation surface.
- [x] Ensure blocked-payment and authorization denials return explanatory states (§6.4), never raw 403 or silent failure. — shipped: `OrderPaymentOpeningNotAuthorisedException` and the `ApplyPaidEffects` precondition exceptions carry explanatory denial results (`ApplyPaidEffectsTest`).
- [x] Ensure duplicate submission renders the same confirmation (§6.6). — shipped: the partial unique index `order_status_events_paid_once` returns the same `Order` on duplicate `DIBAYAR` arrival, and the wizard's double-submit test proves one draft (`BookingWizardEndToEndTest::test_a_double_submitted_step_1_does_not_create_two_drafts_from_one_click`).
