...
- [ ] Allowlist the confirmation-snapshot variables (operator contact, required-document names) for customer confirmation templates. _Requirements: 15_ — **partial**: the variable-resolution **pipeline** shipped on `feat/notification-template-variables` (commits `452e7a8d..cc8907e5`) — `NotificationVariableSource`/`PayloadNotificationVariableSource` (the source seam), `NotificationVariableResolver` (merges sources, restricts the bag to the active template version's `variable_allowlist`, and exposes `appending()` so a feature module can add a source later and win a key collision), registered as a closure singleton in `NotificationServiceProvider`, and wired into all three `TemplateRenderer::render()` call sites (`Actions/DispatchNotification.php`, `Channels/MailChannel.php`, `Channels/LogChannel.php` — previously all three passed `[]`, so no template had ever carried a variable). Evidence: `tests/Unit/Platform/Notification/NotificationVariableResolverTest.php`, `tests/Unit/Platform/Notification/PayloadNotificationVariableSourceTest.php`, `tests/Unit/Platform/Notification/NotificationServiceProviderVariableResolverBindingTest.php`, `tests/Feature/Notification/NotificationTemplateVariablesRenderTest.php` (145/145 in the notification suite, run locally on SQLite; no CI run on this branch yet). The **confirmation-snapshot variables themselves are not started**: no source yet supplies operator-contact or required-document values, and no template references them — that is `docs/superpowers/plans/2026-09-19-order-confirmation-snapshot.md`, a separate plan (PRD amendment 19 Sep 2026, follow-up order 1). The same branch also fixed a pre-existing defect: two version-2 template bodies used single-brace placeholders the renderer never matched (`docs/remediation/findings.yml` `NOTIF-14`).
