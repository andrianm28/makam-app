...
| AC15 | Confirmation-snapshot variables (operator contact, required-document names) allowlisted for customer confirmation templates | `tests/Feature/Notification/NotificationTemplateVariablesRenderTest.php`; `tests/Unit/Platform/Notification/NotificationVariableResolverTest.php`; `tests/Unit/Platform/Notification/PayloadNotificationVariableSourceTest.php`; `tests/Unit/Platform/Notification/NotificationServiceProviderVariableResolverBindingTest.php` (pipeline evidence only — see Status) | Open — the variable-resolution pipeline these tests cover shipped on `feat/notification-template-variables` and passes locally (145/145 in the notification suite, run on SQLite in a container; this branch has no PR and has never run through CI, so it does not meet even this doc's own "CI PostgreSQL evidence still pending" bar for `Closed (local evidence)`, let alone clear it). More fundamentally, AC15 itself — allowlisting the confirmation-snapshot variables (operator contact, required-document names) — has no source and is not started; the tests above prove the pipeline a future source will plug into, not AC15's own requirement. See `docs/superpowers/plans/2026-09-19-order-confirmation-snapshot.md` |
| AC16 | `Reminder due` follows the matrix's window/recipient policy; `window` = reminder window; nearest-future-window only for late due dates | none yet | Open — added 19 Sep 2026 from the PRD reconciliation; not started |
