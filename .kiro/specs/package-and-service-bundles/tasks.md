...
- [ ] Implement quote expansion and price snapshots. _Requirements: 3, 4_ — **partial, and the halves changed on 13 Aug 2026**: the price-*versioning* mechanism is done (`price_versions` schema, `RecordServiceDefinitionPriceVersion`, tested), and **the price snapshot is now real** — `App\Domain\Quotation\Actions\IssueQuote` copies `unit_amount_minor` + `price_version_number` (and `price_version_id`) into immutable `quote_lines` at issuance (`IssueQuote.php:127,130`, `tests/Feature/Quotation/`), so a durable quote-held price reference exists exactly as AC3's snapshot requires. Quote *expansion* (turning a package into real order/quote line items) is **still not built** — `IssueQuote` lines come from the wizard's selected services, not from `service_package_items` — and that half belongs to the booking/orchestration seam (S4-T4/S4-T5 onward).
